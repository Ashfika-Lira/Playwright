...
# @pytest.mark.xfail(reason="BUG 1862")
...
def test_02_logout(set_up_tear_down) -> None:
    ...
    assert login_btn.is_visible(), "Logout unsuccessful"
...
def test_03_login_with_invalid_credentials(set_up_tear_down_no_login) -> None:
    ...
    assert expected_err_text in error_text.inner_text(), "Error not found"


@pytest.mark.regression
@pytest.mark.parametrize("username, password", [("standard_user", "secret_sauce"),
                                                pytest.param("locked_out_user", "secret_sauce", marks=pytest.mark.xfail),
                                                pytest.param("dummy_user", "dummy_password", marks=pytest.mark.xfail)])
