...
def test_click_for_js_confirm(page: Page) -> None:
    ...
    # assert result_msg_loc.inner_text() == 'You clicked: Cancel'
...
def test_click_for_js_prompt(page: Page) -> None:
    ...
    page.on("dialog", lambda dialog: dialog.accept(prompt_text='AshfikaLira'))
    click_js_alrt_btn.click()
    result_msg_loc = page.locator("#result")
    assert result_msg_loc.inner_text() == 'You entered: AshfikaLira'
